...
- Browse existing training files on `/documents` (users) or `/admin/documents` (admins) with scope filters, search, sort, refresh, and reset controls. Both pages show the last refresh time, document counts, and let you copy the stored training text for quick verification.
